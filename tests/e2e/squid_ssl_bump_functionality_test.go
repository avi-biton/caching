...
var _ = Describe("Squid SSL-Bump Functionality", func() {
	...
	BeforeEach(func() {
		...
		// Get the Squid CA certificate from the cert-manager namespace
		By("Getting Squid CA certificate")
		caSecret, err := k8sClient.CoreV1().Secrets("cert-manager").Get(context.Background(), "root-secret", metav1.GetOptions{})
		...
		trustedClient, err = testhelpers.NewTrustedSquidProxyClient(serviceName, namespace, caSecret.Data["tls.crt"])
